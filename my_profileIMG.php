...
if (isset($_POST['img_upload'])) {
  ...
  if ($form_valid1) {
    $sql = "UPDATE users_profile SET profile_image = '$profile_image' WHERE user_id = $uid";
  }
}
...
if (isset($_POST['profile_save'])) {
  ...
  if ($form_valid) {
    ...
    $sql_profile = "UPDATE users_profile SET profile_image = '$profile_image', nickname = '$nickname', country = '$country' WHERE user_id = $my_profile";
...
// $sql = "SELECT e.*,p.*,COUNT(e.employeeID) cn FROM phones p
// JOIN employees e ON p.employeeID=e.`employeeID` AND e.employeeID = '347483647'";
    ...
    <div class="container mt-3 shadow border p-1">
      <div class="mx-auto profile-image-block">
        ...
        <div class="custom-file file-box">
          <!-- ani kan -->
          <form class="" action="" method="POST" novalidate="novalidate" autocomplete="off" enctype="multipart/form-data">
            <div class="row">
              <i class="fas fa-camera fa-2x"></i>
              <input type="file" class="custom-file-input profile-file" id="inputGroupFile01" aria-describedby="inputGroupFileAddon01">
              <input type="submit" name="img_upload" value="Upload Image" class="btn btn-primary">
            </div>
          </form>
        </div>
      </div>
